...
- `--use-importance-selector`: enables a learnable importance map gate for semantic features.
- `--importance-alpha`: gate strength for the importance map.
- `--use-adaptive-snr-codec`: enables continuous SNR-adaptive codec (Mixture-of-Experts style).
- `--snr-encoder-path` and `--snr-decoder-path`: legacy checkpoint paths when the adaptive codec flag is disabled.
